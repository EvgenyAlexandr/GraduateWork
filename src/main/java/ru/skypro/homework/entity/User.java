...
public class User {
    ...
    /** Логин пользователя (email), уникален в системе. */
    @NotBlank
    ...
    /** BCrypt-хеш пароля; в открытом виде не хранится. */
    @NotBlank
    ...
    /** Контактный телефон в формате {@code +7XXXXXXXXXX}. */
    @Size(max = ApiConstants.PHONE_MAX_LENGTH)
    @Column(length = ApiConstants.PHONE_MAX_LENGTH)
    private String phone;

    /** Роль определяет права доступа ({@link Role#USER} или {@link Role#ADMIN}). */
    @NotNull
